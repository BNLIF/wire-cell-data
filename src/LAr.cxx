...
double WireCell::LAr::Diffusion(double T, double E, int flag){
  ...
  double alpha = 2.2-1.5077*Ldensity(T);
  double results;
...
double WireCell::LAr::vDrift(double T, double E){
  double xFit = 0.0938163 - 0.0052563 *(T-87.302) - 0.000146981 *pow(T-87.302,2);
  double muFit = 5.183987 + 0.01447761 * (T-87.302) - 0.0034972*pow(T-87.302,2)
...
double WireCell::LAr::RRLAr(double lambda, double T){
  ...
  double kB = 1.38065e-16;
  double alphaCM = 0.103276;
  double alphaLL = 0.104471;
  
  double omega = 2*3.1415926*c/lambda/1e-7;
...
double WireCell::LAr::IsoCom(double T){
  double rcp = Cp(T);
  double rcv = Cv(T);
  double rss = SpeedofSound(T);
  double rhoL = Ldensity(T);

  double results = 0.1 *rcp/rcv/(1000*rhoL*rss*rss);
  return results;
}
...
double WireCell::LAr::vDarIonV(double T, double E){
  E = E * 1000.;
  double results = 0.432 * E * 0.01 / Viscosity(T);
  return results;
}
