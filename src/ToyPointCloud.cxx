...
std::pair<double, int> WCP::ToyPointCloud::get_closest_2d_dis_index(double x, double y, int plane){
  double query_pt[2];
  query_pt[0] = x;
  query_pt[1] = y;
  int N = 1;
  ...
  if (N>0)
    return std::make_pair(sqrt(out_dist_sqr[0]),ret_index[0]);
  else
    return std::make_pair(1e9,-1);
}
