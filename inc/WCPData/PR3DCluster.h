...
namespace WCP{
  ...
  public:
    ...
    void Update_mcell_cluster_map(std::map<WCP::SlimMergeGeomCell*,WCP::PR3DCluster*>& mcell_cluster_map);

    
    
    void Create_graph();
    void Create_graph(WCP::ToyCTPointCloud& ct_point_cloud);
    ...
    std::vector<SMGCSelection> Examine_graph(WCP::ToyCTPointCloud& ct_point_cloud);

    // new set of function ...
    
    void dijkstra_shortest_paths(WCPointCloud<double>::WCPoint& wcp_source);
    void dijkstra_shortest_paths(WCPointCloud<double>::WCPoint& wcp_source, WCP::ToyCTPointCloud& ct_point_cloud);
    ...
    bool Construct_skeleton(WCP::ToyCTPointCloud& ct_point_cloud);
